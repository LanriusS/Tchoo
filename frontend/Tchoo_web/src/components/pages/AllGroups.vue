<script setup>
import {getAllGroups,getUser,getUsersFromGroup,getGroup,createGroup,fetchUserInfo} from "../../lib/requests"
...
async function importGroupPopUp() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (fields.length >= 6) {
      ...
      if (existingUser[1]==200) {
        ...
        const user = await register(email, "student", name, surname);
        const existingGroup = groups.find(group => group.group_name === groupName);
